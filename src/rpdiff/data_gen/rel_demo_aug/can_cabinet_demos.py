...
def main(args):
    ...
    # pb_client = create_pybullet_client(gui=args.pybullet_viz, opengl_render=True, realtime=True, server=False)
    ...
    # config_fname = osp.join(path_util.get_rpdiff_config(), 'scene_demo_cfgs', args.config)
    ...
    mesh_data_dirs = {
        ...
        'syn_cabinet': 'syn_can_cabinet_obj_unnormalized',
        'syn_cabinet_packed_nonuniform': 'syn_can_cabinet_packed_nonuniform_unnormalized',
        'syn_cabinet_packed_uniform': 'syn_can_cabinet_packed_uniform_unnormalized',
        'syn_can': 'syn_cans_obj'
    }
    ...
    upright_orientation_dict = {
        ...
        'syn_cabinet': common.euler2quat([0, 0, 0]).tolist(),
        'syn_cabinet_packed_nonuniform': common.euler2quat([0, 0, 0]).tolist(),
        'syn_cabinet_packed_uniform': common.euler2quat([0, 0, 0]).tolist(),
        'syn_can': common.euler2quat([0, 0, 0]).tolist(),
    }
    ...
    for pc in pcl:
        ...
        if pc_master_dict[pc]['class'] == 'syn_book':
            pc_master_dict[pc]['scale_hl'] = [1.005, 0.995]
            pc_master_dict[pc]['scale_default'] = 1.0
        if pc_master_dict[pc]['class'] == 'syn_cabinet':
            pc_master_dict[pc]['scale_hl'] = [1.005, 0.995]
            pc_master_dict[pc]['scale_default'] = 1.0
        if pc_master_dict[pc]['class'] == 'syn_cabinet_packed_nonuniform':
            pc_master_dict[pc]['scale_hl'] = [1.005, 0.995]
            pc_master_dict[pc]['scale_default'] = 1.0
        if pc_master_dict[pc]['class'] == 'syn_cabinet_packed_uniform':
            pc_master_dict[pc]['scale_hl'] = [1.005, 0.995]
            pc_master_dict[pc]['scale_default'] = 1.0
        if pc_master_dict[pc]['class'] == 'syn_can':
            pc_master_dict[pc]['scale_hl'] = [1.005, 0.995]
            pc_master_dict[pc]['scale_default'] = 1.0
    ...
    eval_cam = None
    # eval_cam = RGBDCameraPybullet(cams._camera_env_argss(), pb_client)
    ...
    while True:
        ...
        for pc in pcl:
            ...
            if args.rand_mesh_scale:
                ...
                if pc == 'child':
                    xy_down_scale = 0.85
                    mesh_scale[0] = mesh_scale[0] * xy_down_scale
                    mesh_scale[1] = mesh_scale[1] * xy_down_scale
        ...
        if (parent_class == 'box_container' and child_class == 'bottle') or \
            ...
            if child_class == 'syn_can':
                child_body_z = common.quat2rot(child_final_pose[3:])[:, 2]
                ...
                child_body_y = common.quat2rot(child_final_pose[3:])[:, 1]
            ...
            child_upright = angle_from_upright < args.upright_ori_diff_thresh
