...
class MLP(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, n_layers):
        ...
        self.hidden_layers = nn.ModuleList([nn.Linear(hidden_dim, hidden_dim) for i in range(n_layers - 1)])
...
def dict_to_gpu(ob: Union[dict, torch.Tensor], to_float: bool=True) -> Union[dict, torch.Tensor]:
    if isinstance(ob, collections.Mapping):
        ...
        if to_float:
            return ob.float().cuda()
        else:
            return ob.cuda()


def center_pcd(pcd, ref_pcd=None):
    """
    Args:
        pcd (torch.Tensor): Point cloud to center, either N x 3 or B x N x 3
        ref_pcd (torch.Tensor): Point cloud whose mean should be used to center, 
            either N x 3 or B x N x 3. If None, will use pcd as the reference

    Returns:
        torch.Tensor: Centered point clouds, either N x 3 or B x N x 3
    """
    if ref_pcd is None:
        ref_pcd = pcd

    if ref_pcd.dim() < 3:
        pcd_means = torch.mean(ref_pcd, 0).repeat(pcd.shape[0])
    else:
        pcd_means = torch.mean(ref_pcd, 1).repeat(1, pcd.shape[1])

    if pcd.dim() < 3:
        # a single N x 3 point cloud
        return pcd - pcd_means.reshape(-1, 3)
    else:
        # over a batch, B x N x 3 point clouds
        return pcd - pcd_means.reshape(-1, pcd.shape[1], 3)


class SinusoidalPosEmb:
    def __init__(self, dim, max_pos=None):
        super().__init__()
        self.dim = dim
